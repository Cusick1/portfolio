...
class ExpansionPanels extends StatefulWidget {
  const ExpansionPanels({super.key, required this.content, this.expandedColor});
...
class _ExpansionPanelsState extends State<ExpansionPanels> {
  ...
  Widget build(BuildContext context) {
    return ListView(padding: EdgeInsets.zero, children: [_panels()]);
  }
  ...
  Widget _panels() {
    return ExpansionPanelList(
      expansionCallback: (int index, bool isExpanded) {
        setState(() {
          _contentList[index].isExpanded = !_contentList[index].isExpanded;
        });
      },
      children: _contentList.map<ExpansionPanel>((PanelContent panelContent) {
        return ExpansionPanel(
          ...
          backgroundColor: panelContent.isExpanded
              ? _expandedColor
